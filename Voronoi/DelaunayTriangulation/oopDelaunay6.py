...
class Cell:
	...
	def clockwiseangle(self, point: Point) -> float:
		origin = self.seed
		refvec = [0, 1]
...
for i1 in range(len(seeds)):
	for i2 in range(i1 + 1, len(seeds)):
		for i3 in range(i2 + 1, len(seeds)):
			...
			if d != 0:
				# if non-zero, we can calculate the circumcenter
				...
				# if the circumcenter is out of the domain, the border verticies will handle it
				# TODO give this info to border verticies calculation
...
for cell in cell_mapping.values():
	...
	cell.verticies = sorted(cell.verticies, key = lambda p: cell.clockwiseangle(p))
